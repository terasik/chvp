...
class VaultData:
  ...
  dict with old passwords:
  vault_data={
    "vault_id1": "old_password1",
    "vault_id2": "old_password2"
  }

  dict with new passwords
  vault_data_new={
    "vault_id1": "new_password1",
...
class YamlVault:
  ...
  def handle_vault_id(self, cipher_text):
    ...
    if len(flsp)==3:
      ...
      raise VaultError(f"wrong vault format (first line): {flsp}")
  ...
  def plain_text(self, plain_text):
    ...
    if plain_text:
      if not self.vault_id:
        raise VaultError("vault-id not provided")
